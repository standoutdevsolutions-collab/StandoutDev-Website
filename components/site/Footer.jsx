    ...
    <footer className="relative z-10 border-t border-white/10 bg-[#070a1f]">
      <div className="mx-auto max-w-7xl px-6 py-20">
        <div className="grid gap-12 md:grid-cols-4">
          <div className="md:col-span-2">
            ...
            <div className="mt-6 flex gap-3">
              {['Dribbble', 'X', 'Instagram', 'GitHub'].map(s => (
                <a key={s} href="#" className="rounded-full border border-white/15 px-4 py-1.5 text-xs text-white/70 hover:bg-white hover:text-[#0a0e27] transition">{s}</a>
              ))}
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-3 text-sm text-white/50">Pune,Maharashtra · Remote-first · Worldwide</p>
          </div>
        </div>
        ...
        <div className="mt-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-t border-white/10 pt-8">
          ...
          <p className="text-xs text-white/40">Building the future, one pixel at a time.</p>
        </div>
      </div>
